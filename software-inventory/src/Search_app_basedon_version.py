...
def getInventory(deviceArray):
    ...
    for i in deviceArray[:20000]:
        ...
        for j in resp["apps"]:
            ...
            softwareDict[j["app_name"]] = j["version"]
            # print(j["app_name"],j["version"])
        if searchApp in tempArr:
            # users_containing.append(i["username"])
            specificApps.append([i["device_id"],i["username"],searchApp,softwareDict[searchApp]])
...
def writeToCsv(inventoryArray):
    cols = ['Device ID','Username','Applications',"Version"]
    with open(f'{today}.csv','w') as f:
        writer = csv.DictWriter(f,fieldnames=cols,delimiter='\t')
        writer.writeheader()
        for i in inventoryArray:
            writer.writerow({'Device ID':i[0],'Username':i[1],'Applications':i[2],'Version':i[3]})
